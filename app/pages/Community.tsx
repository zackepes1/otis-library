import React from 'react'
import styles, { layout } from "../../styles";
import Image from 'next/image';

const Community = () => {
  return (
    <div>
          <div className={` flex flex-1 flex-col text-5xl font-oswald text-otisBlue  p-6 lg:p-24`}>Community Hub
    <p className="font-poppins text-lg text-otisBlue mt-10 leading-10 lg:max-w-[470px]">Forget boring libraries... This ain&apos;t your grandma&apos;s reading room. This is the Otis Library Teen Zone, your ultimate hangout for books, games, tech, and all things awesome. Escape the real world and step into a universe built just for you.</p>
    </div>
        <div className="flex flex-row m-auto justify-center items-center">
    </div>
    </div>
  )
}

export default Community